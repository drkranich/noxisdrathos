...
function Section({ label, children }: { label: string; children: React.ReactNode }) { return <div className="space-y-2"><p className="font-mono text-[10px] uppercase tracking-[0.3em] text-muted-foreground">{label}</p><div className="space-y-2">{children}</div></div>; }
function PreviewImage({ src }: { src?: string | null }) { return <div className="aspect-[16/9] border border-border bg-card relative overflow-hidden">{src ? <img src={src} alt="" className="absolute inset-0 w-full h-full object-cover" /> : <div className="absolute inset-0 grid place-items-center text-muted-foreground"><ImageIcon className="w-8 h-8" /></div>}</div>; }
function AssetLine({ path, onRemove }: { path?: string | null; onRemove: () => void }) { return path ? <div className="border border-border bg-card p-3 flex items-center gap-3 text-xs font-mono"><FileText className="w-4 h-4" /><span className="truncate flex-1">{path.split("/").pop()}</span><button type="button" onClick={onRemove} className="text-muted-foreground hover:text-destructive">remover</button></div> : null; }
function FileButton({ accept, onPick, loadingPct, label }: { accept: string; onPick: (f: File) => void; loadingPct?: number; label: string }) {
