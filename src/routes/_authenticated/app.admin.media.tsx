            ...
            {filtered.map((asset) => {
              ...
              return (
                <tr key={asset.id} className="border-t border-border/60 hover:bg-card/30">
                  ...
                  <td className="p-4"><div className="flex items-center justify-end gap-1"><Button variant="ghost" size="icon" onClick={() => open(asset)} title="Preview"><Eye className="w-4 h-4" /></Button><label className="inline-flex h-9 w-9 items-center justify-center rounded-md hover:bg-accent cursor-pointer" title="Replace"><UploadCloud className="w-4 h-4" /><input type="file" className="hidden" onChange={(e) => { const file = e.target.files?.[0]; if (file) replace(asset, file); e.target.value = ""; }} /></label><Button variant="ghost" size="icon" onClick={() => remove(asset)} title="Delete"><Trash2 className="w-4 h-4" /></Button><Button variant="ghost" size="icon" onClick={() => reprocess(asset)} title="Reprocess"><RefreshCcw className="w-4 h-4" /></Button></div></td>
                </tr>
              );
            })}
